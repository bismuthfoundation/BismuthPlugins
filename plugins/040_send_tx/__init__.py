...
# This is the default mempool location, should not be touched.
...
def filter_send_tx_db(tx_dict):
    ...
    sql_mempool(SQL_INSERT_TX, params, action="execute")
...
def filter_send_tx(tx_dict):
    """
    Sends the signed tx to the local host via a mpinsert message, adds the answer in the
    # TODO: if tx is not signed and we have a wallet, sign
...
def sql_mempool(sql='', params=None, action='execute'):
    """Safely execute the request. Lock is required because could be called from different threads."""
    with MEMPOOL_LOCK:
        if action == 'commit':
            MEMPOOL_CON.commit()
            return
